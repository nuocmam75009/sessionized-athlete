...
export default function UploadPage() {
  ...
  function pickGpxFile() {
    fileInputRef.current?.click()
  }

  function onGpxFileSelected(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      setSelectedId(null)
      setGpxData(String(reader.result))
      setParsed({ id: 'gpx-upload', date: '—', title: file.name, distance: '—', duration: '—', pace: '—', hr: '—', elevation: '—' })
      setStage('parsed')
    }
    reader.readAsText(file)
  }

  function onGpxStats(stats: GpxStats) {
    setParsed((prev) => ({
      ...prev,
      distance: formatDistance(stats.distanceM),
      duration: stats.totalTimeMs > 0 ? formatDuration(Math.round(stats.totalTimeMs / 1000)) : prev.duration,
      elevation: `${Math.round(stats.elevationGainM)}m D+`,
    }))
  }
  ...
  return (
    <div>
      ...
      <p className="opacity-70 mb-6">Attach the .gpx track from your watch or bike computer.</p>

      <input ref={fileInputRef} type="file" accept=".gpx" className="hidden" onChange={onGpxFileSelected} />
      ...
      {stage === 'idle' && (
        <>
          ...
          >
            ...
            <div className="font-semibold">Drag your .gpx file here, or click to browse</div>
            <div className="text-text/60 text-[13px] mt-1">Le parcours s&apos;affichera sur la carte une fois importé.</div>
          </div>
      ...
      {stage === 'parsed' && (
        <div className="max-w-[680px]">
          <div className="flex gap-2 items-start mb-5">
            {gpxData ? (
              <RouteMap gpxData={gpxData} onStats={onGpxStats} className="flex-1 h-[220px] rounded-md" />
            ) : (
              <div className="flex-1 h-[220px] border border-dashed border-divider rounded-md bg-surface flex flex-col items-center justify-center gap-2">
                ...
                <div className="text-text/60 text-[13px]">Route preview — connect location data to render the track</div>
              </div>
            )}
          </div>
